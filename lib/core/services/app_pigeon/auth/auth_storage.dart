...
base class _AuthStorage {
  ...
  Future<AuthStatus> currentAuthStatus() async {
    return await getCurrentAuth().then(
      (currentAuth) => _AuthStatusDecider.get(currentAuth),
    );
  }
  ...
  Future<void> saveNewAuth(SaveNewAuthParams saveAuthParams) async {
    ...
    // Do not rely only on FlutterSecureStorage's platform change listener.
    // That listener is not guaranteed to fire for writes made by this same
    // app instance, so publish the new session immediately after login.
  ...
  Future<void> clearCurrentAuthRecord() async {
    ...
    // Publish logout immediately for the same reason as login above.
